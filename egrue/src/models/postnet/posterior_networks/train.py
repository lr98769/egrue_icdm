...
def compute_loss_accuracy(model, loader):
    ...
    with torch.no_grad():
        ...
        for batch_index, (X, Y) in enumerate(loader):
            ...
            if len(Y.shape)==1:
                Y = Y.unsqueeze(1)
            Y_hot.scatter_(1, Y.type(torch.int64), 1)
            X, Y_hot = X.type(torch.float64).to(device), Y_hot.to(device)
...
def train(model, train_loader, val_loader, max_epochs=200, frequency=2, patience=5, model_path='saved_model', full_config_dict={}):
    ...
    for epoch in tqdm(range(max_epochs), total=max_epochs):
        for batch_index, (X_train, Y_train) in tqdm(enumerate(train_loader), total=num_batches):
            Y_train_hot = torch.zeros(Y_train.shape[0], train_loader.dataset.output_dim)
            if len(Y_train.shape)==1:
                Y_train = Y_train.unsqueeze(1)
            Y_train_hot.scatter_(1, Y_train.type(torch.int64), 1)
            X_train, Y_train_hot = X_train.type(torch.float64).to(device), Y_train_hot.to(device)
        ...
        if epoch % frequency == 0:
            ...
            time_left = (time.time()-start)/(epoch+1)*(max_epochs-epoch-1)
            print(
                "Epoch {} -> Val loss {} | Val Acc.: {} | Time Left: {}s |".format(
                    epoch, round(val_losses[-1], 3), round(val_accuracies[-1], 3), round(time_left, 1)))
