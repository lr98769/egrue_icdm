...
from src.models.postnet.architectures.linear_sequential import linear_sequential
from src.models.postnet.architectures.convolution_linear_sequential import convolution_linear_sequential
from src.models.postnet.architectures.vgg_sequential import vgg16_bn
from src.models.postnet.architectures.resnet_sequential import resnet18
from src.models.postnet.architectures.alexnet_sequential import alexnet
from src.models.postnet.posterior_networks.NormalizingFlowDensity import NormalizingFlowDensity
from src.models.postnet.posterior_networks.BatchedNormalizingFlowDensity import BatchedNormalizingFlowDensity
from src.models.postnet.posterior_networks.MixtureDensity import MixtureDensity
...
class PosteriorNetwork(nn.Module):
    ...
    def forward(self, input, soft_output, return_output='hard', compute_loss=True):
        ...
        if self.no_density:  # Ablated model without density estimation
            ...
            log_q_zk = torch.zeros((batch_size, self.output_dim)).to(zk.device)
            alpha = torch.zeros((batch_size, self.output_dim)).to(zk.device)
            ...
            if isinstance(self.density_estimation, nn.ModuleList):
                for c in range(self.output_dim):
                    ...
                    log_q_zk[:, c] = log_p
                    log_q_zk = log_q_zk.to(zk.device)
